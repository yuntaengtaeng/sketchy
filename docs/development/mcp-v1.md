...
현재 Projection은 기존 Screen의 이름·Purpose와 기존 Element의 이름·설명,
Button style, Section direction을 `Apply to Figma`로 반영한다. 검토 이후 Figma
Project revision이 달라졌거나 Screen/Element 구조, Element type, Action 변경이
섞이면 적용하지 않는다. 반영 후 다시 Export하면 JSON Projection도 `synced`로
닫힌다.
